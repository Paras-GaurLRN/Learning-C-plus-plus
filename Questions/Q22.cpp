...
class Matrix
{
    ...
    bool isSparse()
    {
        std::cout << "Rule: A Matrix is Sparse if there are atleast same number of zero entries\n";
        int cnt = 0;
        for(Index Riter = 0; Riter < __R; ++Riter)
        {
        for(Index Citer = 0; Citer < __C; ++Citer)
...
int main()
{
    ...
    bool Spar = Mtx.isSparse();
    if(Spar){std::cout << "The Given Matrix Is A Sparse Matrix!" << std::endl;}
    else{std::cout << "The Given Matrix Is Not A Sparse Matrix!" << std::endl;}
